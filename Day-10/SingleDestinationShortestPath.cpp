...
public:
    ...
    int totalVertices();\
    void dijkstraAlgorithm(); // Single Source Shortest Path
    void reverseDijkstraAlgorithm(); // Single Destination Shortest Path 
};
...
void Graph::reverseDijkstraAlgorithm(){
    int v = this->totalVertices();
    ...
    int des;
    cout<<"Please enter your Destination Node:- ";
    cin>>des;
    int* dis = new int[v];
    ...
    while(count<v){
        ...
        for(int i=0;i<v;i++){
            if(visited[i]==false){
                if(reverseGraph[u][i]!=0 && dis[i]>(reverseGraph[u][i]+dis[u])){
                    dis[i] = dis[u] + reverseGraph[u][i];
                }
            }
        }
        count++;
    }
    cout<<"\nShortest Distance from All Other Node to Destination Node:"<<des<<" is given below:- \n";
    for(int i=0;i<v;i++){
        cout<<i<<" -> "<<des<<": "<<dis[i]<<endl;
    }
    return;
}
...
int main(){
    ...
    cout<<"This Program is dedicated to explore Single Destination Shortest Path Problem using Dijkstra Algorithm\n";
    Graph G = makeGraph();
    G.displayGraph();
    G.reverseDijkstraAlgorithm();
    return 0;
}
